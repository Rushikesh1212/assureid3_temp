...
export default class DeleteCandidate extends TrackerReact(React.Component) {
  constructor(props){
    super(props); 
    this.state ={ 
      "subscription" : {
  ...
  empVerifDelete(event){
    ...
    Meteor.call("deleteUserFromComapnyDetails",id,index,function(error,result){
        if(error){
          ...
          // if (index == 0) {
          //   Meteor.call("removeTempOrder",id);
          // }
          $('#deleteCandidate-'+index).removeClass('in');
          $('#deleteCandidate-'+index).css('display','none');
          $('.modal-backdrop').hide();
