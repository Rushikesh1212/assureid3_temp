...
class CorporateOrderAllocated extends TrackerReact(Component){
	constructor(props){
   super(props);
      this.state = {
      } 
                ...
                <div className="col-md-12">
                  <div className="box">
                    <div className="box-header with-border">
                      <h2 className="box-title"> Allocated Orders</h2> 
                    </div>
                              ...
                              {this.props.allocatedCorporateOrders ? 
                                 this.props.allocatedCorporateOrders.length > 0 ?
                                   this.props.allocatedCorporateOrders.map((orders,index)=>{
                                      return (
                                        <tr key={index}>
                                          <td><a href={orders.DEOStatus == "New" ? "/newRequest/"+orders._id+"/newOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId : "/newRequest/"+orders._id+"/detailOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId}>{orders.corporateOrderNo}</a></td>
                                          <td><a href={orders.DEOStatus == "New" ? "/newRequest/"+orders._id+"/newOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId : "/newRequest/"+orders._id+"/detailOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId}>{orders.companyDetails.companyName}<br/>({orders.companyDetails.companyAssureID})</a></td>
                                          <td><a href={orders.DEOStatus == "New" ? "/newRequest/"+orders._id+"/newOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId : "/newRequest/"+orders._id+"/detailOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId}>{moment(orders.createdAt).format("DD MMM YYYY")}</a></td>
                                          <td><a href={orders.DEOStatus == "New" ? "/newRequest/"+orders._id+"/newOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId : "/newRequest/"+orders._id+"/detailOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId}>{moment(orders.dueDate).format("DD MMM YYYY")}</a></td>
                                          <td><a href={orders.DEOStatus == "New" ? "/newRequest/"+orders._id+"/newOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId : "/newRequest/"+orders._id+"/detailOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId}>{orders.ageing}</a></td>
                                          <td className={"bgtextcolor "+orders.bgClassName}><a href={orders.DEOStatus == "New" ? "/newRequest/"+orders._id+"/newOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId : "/newRequest/"+orders._id+"/detailOrder/"+orders.companyDetails.companyAssureID+"/"+orders.packageDetails.servicesIncluded[0].serviceId}>{orders.status}</a></td>
                                        </tr>
                                      );
                                   })
                                 : 
...
CorporateOrderAllocatedContainer = withTracker(props => { 
    const handleAllOrdersList = Meteor.subscribe("allocatedCorporateOrders",Meteor.userId());
    const loading             = !handleAllOrdersList.ready();
    const allocatedCorporateOrders = CorporateOrders.find({"allocatedToUserid" : Meteor.userId()}).fetch();
    var addNoOfDays = 2;

    for(var i=0;i<allocatedCorporateOrders.length;i++){
...
})(CorporateOrderAllocated);
export default CorporateOrderAllocatedContainer;
