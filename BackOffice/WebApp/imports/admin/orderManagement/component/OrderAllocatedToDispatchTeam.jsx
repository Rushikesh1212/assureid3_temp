...
class OrderAllocatedToDispatchTeam extends TrackerReact(Component){
	constructor(props){
        super(props);
        this.state = {
        } 
    ...
    allTicketListDetails(ticketId){
      ...
      if(allTicketList){
        ...
        console.log("tickteObj :",tickteObj);
      }//EOF allTicketList
...
OrderAllocatedToDispatchTeamContainer = withTracker(props => { 
    ...
    if(allOrderList){
        for(i = 0 ; i < allOrderList.length ; i++){
          ...
          for( j = 0 ; j < allOrderList[i].candidateDetails.length; j++){
              // if(allOrderList[i].candidateDetails[j].candidateVerificationStatus !="Completed"){
...
})(OrderAllocatedToDispatchTeam);
export default OrderAllocatedToDispatchTeamContainer;
