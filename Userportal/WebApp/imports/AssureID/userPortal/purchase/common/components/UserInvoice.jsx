...
class UserInvoice extends TrackerReact(Component){
  constructor(){
    super();
    this.state ={
      invoiceNo       : '',
  ...
  // create canvas object
   getCanvas(cachewidth){
      html2canvas($('#outerInvoiceBlock').get(0)).then( function (canvas) {
      var outerInvoiceBlock = $('#outerInvoiceBlock');
      ...
      if(canvas.width > canvas.height){
        ...
        var doc = new jsPDF('p', 'mm','a4',[canvas.height, canvas.width]);
          ...
          outerInvoiceBlock.width(cachewidth);
    });
...
export default InvoiceContainer = withTracker(({params}) => {
