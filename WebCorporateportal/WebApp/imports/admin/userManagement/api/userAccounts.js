...
if (Meteor.isServer) {
  ...
  Meteor.publish('notaUser',function(emailId){
  // you should restrict this publication to only be available to admin users
    return Meteor.users.find({"emails.address" : emailId, "roles" : {$in:["backofficestaff","superAdmin", "admin", "companyuser"]}});
  });
...
Meteor.methods({
  ...
  'userCreateAccountByCompany' : function(formValues) {
    // console.log(formValues.signupPassword);
    // console.log(formValues.signupPassword.split("").reverse().join(""));
    var newUserId = Accounts.createUser({
      username        : formValues.signupEmail,
      ...
      profile     : {   
        ...
        lastname      : formValues.lastname,
        mobNumber     : formValues.mobNumber,
        aadharCard    : formValues.aadharCard,
  ...
  checkEmailVerification: function(email) {
    ...
    if(found_user){
     var emailData = found_user.emails.filter(function (q) { return q.address == email; });
      if (emailData) {
        if(emailData[0].verified == true){
          return "verified";
        }else{
          return "unverified";
        }
  ...
  checkAssureidandemailid: function(email,assureId) {
    found_user = Meteor.users.findOne({'profile.assureId' : assureId ,'emails.address' : email})
    if (found_user) {
      Meteor.users.update(
        {'_id': found_user._id },
        {
          $set:{
            "profile.loggedinEmail"        :  email,
          } //End of set   
        }
      );
      return "Correct";
  ...
  'resetPasswordUsingotp': function(id, password){
    ...
    var reversePass  = reverseArray.join("");     

    Accounts.setPassword(id, password);
    Meteor.users.update({"_id" : id},{
     $set : {
      "profile.changedPassword" : true,
      "profile.userCode"        : reversePass,
     }
    });
  },
  "notaUser" :function(emailId){
    var userValid  = Meteor.users.findOne({"emails.address" : emailId, "roles" : {$in:["backofficestaff","superAdmin", "admin", "companyuser"]}});
     // if (userValid) {
...
Meteor.startup(() => {
    ...
    superUserId = Accounts.createUser({
              ...
              password : 'superAdmin',
              profile  : { 'status' : 'Active','firstname' : 'Super', 'lastname' : 'Admin' },
          });
