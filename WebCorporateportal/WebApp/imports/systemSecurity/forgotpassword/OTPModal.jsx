...
export default class OTPModal extends React.Component{
    ...
    if($('#OTPMobMail').valid()){
      ...
      if(emailotp && mobileotp){
        if(mailotp == emailotp && mobotp == mobileotp){
          ...
          swal({
            title: "Incorrect",
            text: 'Either email OTP or mobile OTP is incorrect',
  ...
  resendOTP(event){
    ...
    var sessionEmailId  = Session.get('emailId');
    var sessionMobileno = Session.get('mobileNo');
    // console.log("sessionValue",sessionValue);
    if(sessionValue && sessionEmailId && sessionMobileno){
      var newID    = sessionValue;
      var userData = Meteor.users.findOne({"_id":newID});
      if(userData){
        if (userData.profile) {
          if (userData.profile.authorizedPerson) {
            var matchedSPOCperson = userData.profile.authorizedPerson.find((obj)=>{return obj.address == sessionEmailId && obj.accessPersonContact == sessionMobileno});
            if (matchedSPOCperson) {
              var mobile   = matchedSPOCperson.accessPersonContact;
              var username = matchedSPOCperson.address;  
              ...
              if(mobile){
                ...
                // console.log("mobileotpStr",mobileotpStr);
                ...
                Meteor.call('SEND_SMS',mobile,smsBody,function(error,result){
                  ...
                  swal({
                    title: "Incorrect",
                    text: 'Your mobile number is not found. Please enter valid mobile number.',
                    showConfirmButton: true,
                    showCancelButton: false,
                    confirmButtonColor: '#666',
                    type: 'error'
              ...
              // swal('Successfully sent the OTP to your mobile number and email address.');
            }else{
              swal({
                title: "Incorrect",
                text: 'Inputs not found. Please enter valid Email ID and Mobile number.',
                showConfirmButton: true,
                showCancelButton: false,
                confirmButtonColor: '#666',
                type: 'error'
              });
              // swal('Inputs not found. Please enter valid Email ID and Mobile number.');
            }
          }
        }
      }
